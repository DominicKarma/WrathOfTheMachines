...
using WoTM.Content.NPCs.ExoMechs.ComboAttacks.AresAndTwins;
...
namespace WoTM.Content.NPCs.ExoMechs.ComboAttacks.HadesAndTwins
{
    public class ExoelectricPortalDashes : ExoMechComboHandler
    {
        ...
        /// </summary>
        public static readonly SoundStyle HeavyPortalWarpSound = new("WoTM/Assets/Sounds/Custom/ExoTwins/PortalWarpHeavy");
        ...
        public static bool Perform_Hades(NPC npc)
        {
            ...
            hades.BodyBehaviorAction = new HadesHeadBehavior.BodySegmentInstructions(HadesHeadBehavior.AllSegments(), segment =>
            {
                ...
                HadesHeadBehavior.CloseSegment().Invoke(segment);
        ...
        public static bool Perform_Hades_PerformPortalDashes(NPC npc, ref float wentThroughPortal)
        {
            ...
            if (AITimer == ExoTwinHyperfuturisticPortal.Lifetime / 2 + PortalEnterWaitDelay + 10)
            {
                ...
                SoundEngine.PlaySound(HadesHeadBehavior.SideLaserBurstSound with { MaxInstances = 0 }).WithVolumeBoost(2f);
                ...
                if (Main.netMode != NetmodeID.MultiplayerClient && portal is not null)
                {
                    foreach (NPC n in Main.ActiveNPCs)
                    {
                        ...
                        if (n.whoAmI == npc.whoAmI || n.realLife == npc.whoAmI || isExoTwin)
                        {
                            ...
                            if (n.realLife == npc.whoAmI && n.TryGetBehavior(out HadesBodyBehavior body))
                            {
