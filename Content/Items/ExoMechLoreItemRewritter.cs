...
namespace WoTM.Content.NPCs.ExoMechs
{
    public class ExoMechLoreItemRewritter : GlobalItem
    {
        ...
        {
            HookHelper.ModifyMethodWithIL(typeof(Main).GetMethod("MouseText_DrawItemTooltip", BindingFlags.Instance | BindingFlags.NonPublic), DrawBlackBoxForLoreItem);
        }
        ...
        public override void ModifyTooltips(Item item, List<TooltipLine> tooltips)
        {
            if (item.type == ModContent.ItemType<LoreExoMechs>())
            {
                tooltips.RemoveAll(t => t.Name.Contains("Tooltip") || t.Name.Contains("Lore"));
            }
        }
