...
float4 PixelShaderFunction(float4 sampleColor : COLOR0, float2 coords : TEXCOORD0) : COLOR0
{
    ...
    float4 color = Overlay(baseColor, electricityColor) * baseColor.a + pow(baseColor.r, 1.5) + pow(baseColor.b, 3) * 5;
    return lerp(baseColor, saturate(color), electrifyInterpolant) * sampleColor;
}
